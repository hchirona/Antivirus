...
sub recursion {
	...
	print STDERR "$dir: Permiso denegado: $!, Continuando\n";
	return;
    }
    @list = readdir(I);
    closedir(I);
...
######################################################################################
	    print "$currentfile: Sospechoso($pat):\n $string\n\n";
	    last;
	}
