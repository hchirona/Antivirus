...
class acymailingPHPMailer {
	...
	function SmtpSend($header, $body) {
		...
		if(!$this->smtp->Mail($smtp_from)) {
			$errorInfos = empty($this->smtp->error['error']) ? $this->Lang('from_failed').$smtp_from : $this->smtp->error['error'];
		...
		if(count($bad_rcpt) > 0) { // Create error message
			...
			$errorInfos = empty($this->smtp->error['error']) ? $this->Lang('recipients_failed') : $this->smtp->error['error'];
			$errorInfos .= ' ( '.$error.' ) ';
	...
	function SmtpConnect() {
			...
			if($tls) {
				...
				if($this->SMTPAuth) {
					if(!$this->smtp->Authenticate($this->Username, $this->Password)) {
						$errorInfos = empty($this->smtp->error['error']) ? $this->Lang('authenticate') : $this->smtp->error['error'];
						//Then we add additional data...
						if(!empty($this->smtp->error['smtp_code'])) $errorInfos .= ' '.$this->smtp->error['smtp_code'];
						if(!empty($this->smtp->error['smtp_msg'])) $errorInfos .= ' '.$this->smtp->error['smtp_msg'];
		...
		if(!$connection) {
			$errorInfos = empty($this->smtp->error['error']) ? $this->Lang('connect_host') : $this->smtp->error['error'];
			//Then we add additional data...
			if(!empty($this->smtp->error['smtp_code'])) $errorInfos .= ' '.$this->smtp->error['smtp_code'];
			if(!empty($this->smtp->error['smtp_msg'])) $errorInfos .= ' '.$this->smtp->error['smtp_msg'];
